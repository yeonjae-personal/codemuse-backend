<template>
  <SearchPane
    ...
    :model-list="entitySearchCreate.items || []"
    :pagination="entitySearchCreate?.pagination"
  ...
  >
    <template #custom-form>
      <div class="flex flex-column mt-2 gap-2">
        <div class="grid grid-cols-2 gap-2">
          <BaseSelectScroll
            v-model="paramsSearchEntityCreate.itemCode"
          ...
          />
          <BaseSelectScroll
            v-model="paramsSearchEntityCreate.entityTypeCode"
            :options="subTypeList"
        ...
        <div class="grid grid-cols-[1fr_2fr] gap-2">
          <BaseSelectScroll
            v-model="entitySearchNmCdCreate"
    ...
    <template #custom-search-item="{ item, index }">
      <cf-card-dropdown
        ...
        :search-field="entitySearchNmCdCreate"
        type-bg="linear"
      ...
      >
        ...
        <template #detail>
          <div class="flex flex-column gap-4">
            <ProductGrid v-if="item.detail" :data="item.detail" />
          </div>
        </template>
      </cf-card-dropdown>
    </template>
...
import { useComponentStore, useDragStore } from "@/store";
import { ITEMS_PAGE_TYPE, NM_CD_FIELDS } from "@/constants/";
import {
...
const { getEntityList, resetParamListEntitySearch } = useComponentStore();
...
const subTypeList = computed(() => {
  if (
    paramsSearchEntityCreate.value.itemCode &&
    multiEntityTypes.value?.length
  ) {
    const list = multiEntityTypes.value.find(
      (item) => item.value === paramsSearchEntityCreate.value.itemCode
    );
...
const checkItemDragExist = computed(() => (value: string) => {
  let arrs = [] as any;
  componentCreateEntityList.value.forEach((item) => {
    if (item.objRel?.length) {
      arrs = [...arrs, ...item.objRel];
    }
  });
...
const handleSearch = async (
  ...
  if (entitySearchNmCdCreate.value === NM_CD_FIELDS[0].value) {
    paramsSearchEntityCreate.value.multiEntityName = inputValue.value;
  } else {
    paramsSearchEntityCreate.value.multiEntityCode = inputValue.value;
  }
  paramsSearchEntityCreate.value.page = isClick ? 1 : page;
  paramsSearchEntityCreate.value.size = pageSize
    ? pageSize
  ...
  await getEntityList(ITEMS_PAGE_TYPE.CREATE);
};

const handleChangeNmCd = (): void => {
  if (entitySearchNmCdCreate.value === NM_CD_FIELDS[0].value) {
    paramsSearchEntityCreate.value.multiEntityCode = null;
    paramsSearchEntityCreate.value.multiEntityName = inputValue.value;
  } else {
    paramsSearchEntityCreate.value.multiEntityCode = inputValue.value;
    paramsSearchEntityCreate.value.multiEntityName = null;
  }
};
...
const handleChangePage = async (page: number): Promise<void> => {
  paramsSearchEntityCreate.value.page = page;
  await getEntityList(ITEMS_PAGE_TYPE.CREATE);
};

...
const searchType = ref({ label: "전체", value: "" })
const srchWord = ref("");
...
const searchTypeOption = ref([
  { label: "전체", value: "" },
  { label: "코드그룹명", value: "grpNm" },
  { label: "코드상세명", value: "detlNm" },
]);

const handleClickSearch = () => {
  emit("search", {
    srchType: searchType.value.value,
      ...
      <v-row no-gutters>
        <v-col md="3" sm="6">
          ...
          >
            <label class="no-wrap-label">{{
              $t("code.code_search.lbl_use_yn")
            }}</label>
            <v-form ref="formSearch">
              <div class="flex w-[140px]">
                <v-combobox
                  v-model="searchType"
                  :items="searchTypeOption"
        ...
        <v-col md="4" sm="6" xs="12">
          <div class="flex flex-nowrap">
            <label class="mt-6">
              {{ $t("code.code_search.lbl_srch_word") }}
            </label>
        ...
        <v-col md="1" class="d-flex justify-end align-center sm:pb-4">
          <cf-button
            :label="$t(`code.code_search.btn_search`)"

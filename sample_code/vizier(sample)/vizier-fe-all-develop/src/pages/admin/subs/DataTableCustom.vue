  ...
  <div>
    ...
    >
      ...
      <template #[`item.select`]="{ item }">
        <SearchIcon size="18" fill="#6B6D70" />
        <p>{{ item.name }}</p>
      </template>
      <SelectionIcon size="18" fill="#6B6D70" :selected="true" />

      <template #[`item.no`]="{ item }">
        <SearchIcon size="18" fill="#6B6D70" />
        <p>{{ item.name }}</p>
      </template>
      <template #[`item.codeNo`]="{ item }">
        <SelectionIcon
          size="18"
          fill="#6B6D70"
          :selected="item == idSelected"
        />
        <p>{{ item.name }}</p>
      </template>
      <template #[`item.screenNo`]="{ item }">
      ...
      <template #[`item.enableDisable`]="{ item }">
        <div class="flex justify-center items-center">
          <v-switch
            v-model="item.enableDisable"
            hide-details
            class="custom-switch"
      ...
      <template #[`item.sysMessageNo`]="{ item }">
        <SelectionIcon
          size="18"
          fill="#6B6D70"
          :selected="item == idSelected"
      ...
      <template #[`item.permissionNo`]="{ item }">
        <SelectionIcon
          size="18"
          fill="#6B6D70"
          :selected="item == idSelected"
    ...
    >
      <div class="flex align-center">
        ...
        <span v-if="totalItems > 0" class="text-[12px] text-text-lighter"
          >{{ startRange }}-{{ endRange }} {{ $t("product_platform.of") }}
          {{ totalItems }}</span
        >
        <span v-else class="text-[12px] text-text-lighter"
          >0-0 {{ $t("product_platform.of") }} 0</span
        >
      </div>
...
:deep(.selected-row) {
  background-color: #ba1642 !important;
}
</style>
...
:dep(.v-switch--inset .v-switch__thumb){
  height: 20px;
  width: 20px;
}

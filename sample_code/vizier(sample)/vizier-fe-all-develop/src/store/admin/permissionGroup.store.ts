import { getPermissionGroupListApi } from "@/api/admin/adminApi";
import { PermissionGroupSearchParam } from "@/interfaces/admin/admin";
...
const usePermissionGroupStore = defineStore("permissionGroup", {
  ...
  actions: {
    async fetchPermissionGroup(params?: PermissionGroupSearchParam) {
      try {
        const response = await getPermissionGroupListApi(params);
...
export default usePermissionGroupStore;

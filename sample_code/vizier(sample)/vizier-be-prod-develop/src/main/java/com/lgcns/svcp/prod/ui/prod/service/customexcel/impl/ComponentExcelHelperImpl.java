...
public class ComponentExcelHelperImpl extends ExcelHelperImpl implements ComponentExcelHelper {
	...
	public void createSheet(ExcelWriter excelWriter, ExcelInput excelInput) {
		...
		// Create header
		Object header = null;
		List<ComponentStructureExportDto> structures = null;
		...
		if (excelInput.getObject() instanceof Object[]) {
			...
			header = objects[0];
			if (objects[1] != null) {
				structures = (List<ComponentStructureExportDto>) objects[1];
			}
			if (objects[2] != null) {
				multiEntities = (List<MultiEntityExportDto>) objects[2];
			}
			itemCode = (String) objects[3];
			itemName = (String) objects[4];
		}
		...
		// Create body
		List<ComponentExportDto> datas = (List<ComponentExportDto>) excelInput.getDatas();

		for (int iny = 0, rowSize = datas.size(); iny < rowSize; iny++) {
			Row valueRow = sheet.createRow(iny + 3);
			ComponentExportDto data = datas.get(iny);
		...
		String[] headerStructures = createTitleHeaderStructure();
		excelWriter.setAutoCellSize(true);
		excelWriter.setTitles(headerStructures);
		excelWriter.createSheet(structures, "Structure");
		if (itemCode.equalsIgnoreCase("BN") || itemCode.equalsIgnoreCase("CH")) {
	...
	private String[] createTitleHeaderStructure() {
		if (ComponentStructureExportDto.class.isAnnotationPresent(CustomTitleHeader.class)) {
			List<String> results = new ArrayList<>();
			Field[] fields = ComponentStructureExportDto.class.getDeclaredFields();
			for (Field field : fields) {
				field.setAccessible(true);
				if (field.isAnnotationPresent(Value.class)) {
					Value value = field.getAnnotation(Value.class);

...
public class GroupExcelHelperImpl extends ExcelHelperImpl implements GroupExcelHelper {
	...
	public void createSheet(ExcelWriter excelWriter, ExcelInput excelInput) {
		...
		// Create header
		Object header = null;
		List<GroupStructureExportDto> structures = null;
		String itemName = null;
		if (excelInput.getObject() instanceof Object[]) {
			Object[] objects = (Object[]) excelInput.getObject();
			header = objects[0];
			if (objects[1] != null) {
				structures = (List<GroupStructureExportDto>) objects[1];
			}
		...
		// Create body
		List<GroupExportDto> datas = (List<GroupExportDto>) excelInput.getDatas();
 
		for (int iny = 0, rowSize = datas.size(); iny < rowSize; iny++) {
			Row valueRow = sheet.createRow(iny + 3);
			GroupExportDto data = datas.get(iny);
	...
	private String[] createTitleHeaderStructure() {
		if (GroupStructureExportDto.class.isAnnotationPresent(CustomTitleHeader.class)) {
			List<String> results = new ArrayList<>();
			Field[] fields = GroupStructureExportDto.class.getDeclaredFields();

...
public class OfferListExcelHelperImpl extends ExcelHelperImpl implements OfferListExcelHelper {
	...
	public void createSheet(ExcelWriter excelWriter, ExcelInput excelInput) {
		...
		// Create header
		Object header = null;
		List<OfferStructureExportDto> structures = null;
		String itemName = null;
		if (excelInput.getObject() instanceof Object[]) {
			Object[] objects = (Object[]) excelInput.getObject();
			header = objects[0];
			if (objects[1] != null) {
				structures = (List<OfferStructureExportDto>) objects[1];
			}
			itemName = (String) objects[2];
		}
		...
		// Create body
		List<OfferExportDto> datas = (List<OfferExportDto>) excelInput.getDatas();

		for (int iny = 0, rowSize = datas.size(); iny < rowSize; iny++) {
			Row valueRow = sheet.createRow(iny + 3);
			OfferExportDto data = datas.get(iny);
	...
	private String[] createTitleHeaderStructure() {
		if (OfferStructureExportDto.class.isAnnotationPresent(CustomTitleHeader.class)) {
			List<String> results = new ArrayList<>();
			Field[] fields = OfferStructureExportDto.class.getDeclaredFields();
